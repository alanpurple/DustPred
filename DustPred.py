...
def inference(path,model):
    ...
    pred=model.predict({'dust_input':test_dust,'region_input':region})
...
def get_simplegru_model(num_units=64,num_layers=2,dropout=0.1):
    inputs=Input((5,2),name='dust_input')
    region=Input((),name='region_input',dtype=tf.uint8)
    month=Input((),name='month_input',dtype=tf.uint8)
    ...
    multi_gru=layers.LSTM(num_units)

    output=multi_gru(inputs)
